...
export function applyAppearanceToDocument(
  ...
  // Default palette: emerald accents in light mode, brand accents in dark mode.
  const themeId =
    state.palette === "default" && mode === "light" ? "emerald" : state.palette
  ...
  const scale = FONT_SCALE_BY_ID[state.fontScale]
...
export const APPEARANCE_INIT_SCRIPT = `(function(){try{var p=localStorage.getItem("${APPEARANCE_STORAGE_KEYS.palette}")||"${DEFAULT_APPEARANCE.palette}";var n=localStorage.getItem("${APPEARANCE_STORAGE_KEYS.neutral}")||"${DEFAULT_APPEARANCE.neutral}";var f=localStorage.getItem("${APPEARANCE_STORAGE_KEYS.font}")||"${DEFAULT_APPEARANCE.font}";var r=localStorage.getItem("${APPEARANCE_STORAGE_KEYS.radius}")||"${DEFAULT_APPEARANCE.radius}";var cv=localStorage.getItem("${APPEARANCE_STORAGE_KEYS.colorVision}")||"${DEFAULT_APPEARANCE.colorVision}";var fs=localStorage.getItem("${APPEARANCE_STORAGE_KEYS.fontScale}")||"${DEFAULT_APPEARANCE.fontScale}";var rv={default:"0.625rem",soft:"0.875rem",sharp:"0.375rem"};var fsv={"100":"1","112":"1.12","125":"1.25","150":"1.5"};var sc=fsv[fs]||fsv["100"];var root=document.documentElement;var t=localStorage.getItem("theme")||"system";var prefersDark=window.matchMedia("(prefers-color-scheme: dark)").matches;var mode="light";if(t==="dark"){mode="dark";}else if(t==="system"){mode=prefersDark?"dark":"light";}var themeId=(p==="default"&&mode==="light")?"emerald":p;root.setAttribute("data-theme",themeId);root.setAttribute("data-neutral",n);root.setAttribute("data-font",f);root.setAttribute("data-radius",r);root.setAttribute("data-color-vision",cv);root.setAttribute("data-font-scale",fs);root.style.setProperty("--radius-base",rv[r]||rv.default);root.style.setProperty("--text-scale",sc);root.style.setProperty("--font-scale",sc);root.classList.remove("light","dark");root.classList.add(mode);${CUSTOM_BRAND_INIT}${COLOR_VISION_INIT}${FAVICON_INIT}}catch(e){}})();`
