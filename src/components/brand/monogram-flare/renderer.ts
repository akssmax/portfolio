...
import { readThemeRgb, themeLuminance } from "@/lib/brand/runner-canvas-theme";
...
export function createRenderer({
  ...
  const frameLoop = (now: number) => {
    ...
    guard(() => {
      ...
      const backgroundColor = readThemeRgb(host, "--background");
      const lightTheme = themeLuminance(backgroundColor) > 0.45 ? 1 : 0;
      activePipeline.setFrameUniforms(
        ...
        pulseHold,
        {
          backgroundColor,
          flareColor:
            lightTheme === 1 ? readThemeRgb(host, "--primary") : FLARE_COLOR,
          lightTheme,
        }
      );
  ...
  const initialize = async () => {
    ...
    if (disposed) return;
    const nextGpu = await init({ label: "monogram-flare" });
    if (disposed) {
      try {
        nextGpu.dispose();
      } catch {
