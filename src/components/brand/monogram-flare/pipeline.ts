...
const GLYPH_CENTER_IN_BOX: Point = [0.5, 0.62];
const LOGO_ASPECT = 205 / 153;
const LOGO_HEIGHT_RATIO = 0.62;
const MAX_RENDER_WIDTH = 1920;
const PULSE_TRANSITION_SECONDS = 2;
const PULSE_FLOOR = 0.2;
export const FLARE_COLOR = [179 / 255, 191 / 255, 1] as const;
...
export type FlareAppearance = {
  readonly backgroundColor: readonly [number, number, number];
  readonly flareColor: readonly [number, number, number];
  readonly lightTheme: number;
};
...
export class FlarePipeline {
  ...
  constructor(private readonly gpu: Gpu, private readonly output: Target) {
    ...
    try {
      this.effects = {
        logo: effect(gpu, fullscreen(logoWgsl), { label: "monogram-flare-logo" }),
        rim: effect(gpu, fullscreen(rimWgsl), { label: "monogram-flare-rim" }),
        rimBlurH: effect(gpu, fullscreen(blurWgsl), {
          label: "monogram-flare-rim-horizontal",
        }),
        rimBlurV: effect(gpu, fullscreen(blurWgsl), {
          label: "monogram-flare-rim-vertical",
        }),
        composite: effect(gpu, fullscreen(compositeWgsl), {
          label: "monogram-flare-composite",
        }),
      };
    } catch (error) {
  ...
  async replace(
    ...
    try {
      ...
      const nextTexture = this.gpu.gpu.createTexture({
        label: "monogram-flare-logo-raster",
  ...
  setFrameUniforms(
    ...
    this.effects.composite.set({
      params: {
        ...
        aspect: placement.canvasToLogo,
        logoCenter: placement.logoCenter,
        flareColor: appearance.flareColor,
        rimIntensity: 0.62 * attenuation,
        extension: 0.48,
        beamIntensity: 0.48 * attenuation,
        filmGrain: 0.03,
        smoothness: 1,
        logoOpacity: 1,
        frameIndex,
...
function createBlueNoiseTexture(gpu: Gpu): GPUTexture {
  const texture = gpu.gpu.createTexture({
    label: "monogram-flare-blue-noise-128",
