...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let plate = color * edgeMask;
  let lum = dot(plate, vec3f(0.2126, 0.7152, 0.0722));
  let glow = clamp(lum * 1.75, 0.0, 1.0);
  let glyph = scene * edgeMask * params.logoOpacity;
  var onLight = mix(params.backgroundColor, params.flareColor, glow);
  onLight = mix(onLight, params.flareColor * 0.38, glyph);
  let onDark = params.backgroundColor + plate;
  let composed = mix(onDark, onLight, params.lightTheme);
