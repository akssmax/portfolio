...
function prepColors(input?: string[]) {
  ...
  for (let i = 0; i < MAX_COLORS; i++) {
    arr.push(hexToRGB(base[Math.min(i, base.length - 1)] ?? base[0] ?? "#ffffff"))
  }
...
export type LightfallProps = {
  ...
  /** Track pointer on a parent element (e.g. hero section) instead of the canvas. */
  pointerRootRef?: RefObject<HTMLElement | null>
}
...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    uniformsRef.current = { iMouse: uniforms.iMouse, iTime: uniforms.iTime }
    ...
    const pointerTarget = () => pointerRootRef?.current ?? canvas
  ...
  useEffect(() => {
    ...
    if (rafRef.current === 0 && loopRef.current) {
